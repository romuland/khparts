...
function importopt(){	
	...
	/*if(strpos(strtolower($_FILES[$upload_file_field]['name']), "opt.xls")=== false){
		...
		if (move_uploaded_file($_FILES[$UPLOAD_FILE_FIELD]['tmp_name'], $uploaddir.$_FILES[$UPLOAD_FILE_FIELD]['name'])) {
			...
			echo "<p style='color:blue;'>".iconv('UTF-8', 'windows-1251', 'Файл успешно загружен!')."</p>";
			$file_name = $uploaddir.$_FILES[$UPLOAD_FILE_FIELD]['name'];
		} else {
			echo "<p style='color:red;'>".iconv('UTF-8', 'windows-1251', 'Что-то пошло не так при загрузке файла: '.$_FILES[$UPLOAD_FILE_FIELD]['error'])."</p>";
			$upload_failure = true;
		}
	//}
}
