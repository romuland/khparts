	...
	<?php
		if ($app->isAdmin()) {
			...
			if ($user->authorise('core.admin', 1)) {
				$link = JURI::base(true).'/index.php?option=com_plugins&filter_folder=system&filter_search=Sourcerer';
				echo '<em>'.html_entity_decode(JText::_('SRC_SEE_THE_PLUGIN_FOR_SETTINGS'), ENT_COMPAT, 'UTF-8').' (<a href="'.$link.'" target="_blank">'.JText::_('NN_SETTINGS_EDITOR_BUTTON').'</a>)</em>';
			}
		}
